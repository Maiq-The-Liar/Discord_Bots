...
BOT_STATE_SORTING_ANNOUNCEMENT_PREFIX = "sorting_announcement_channel:"
BOT_STATE_QUIZ_MESSAGE_PREFIX = "housing_quiz_message:"
...
def make_base_embed(title: str, description: str) -> discord.Embed:
    embed = discord.Embed(title=title, description=description)
    embed.set_thumbnail(url=DEFAULT_CREST)
    return embed


def make_house_embed(title: str, description: str, house_name: str) -> discord.Embed:
...
def build_question_embed(index: int, qdata: dict) -> discord.Embed:
...
class AnswerButton(discord.ui.Button):
    def __init__(self, cog: "HousingQuizCog", label: str, score_map: dict[str, int], option_index: int):
        super().__init__(
            label=label,
            style=discord.ButtonStyle.primary,
            custom_id=f"housing_quiz_answer_{option_index}",
        )
        self.cog = cog
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        session = self.cog.active_sessions.get(user_id)
        ...
        if session["question_index"] >= len(QUESTIONS):
            guild = interaction.guild or self.cog.bot.get_guild(session["guild_id"])
            if guild is None:
                self.cog.active_sessions.pop(user_id, None)
            ...
            with self.cog.database.connect() as conn:
                ...
                if role_service.member_has_house(guild, member):
                    self.cog.active_sessions.pop(user_id, None)
                    await interaction.response.edit_message(
                ...
                house_role = role_service.get_house_role(guild, winning_house)
            ...
            try:
                await member.add_roles(house_role, reason="Housing quiz result")
            except discord.HTTPException as exc:
                await interaction.response.edit_message(
                    content=f"Your house is **{winning_house}**, but I couldn't assign the role: {exc}",
                    embed=None,
                    view=None,
                )
                return
            ...
            await self.cog.send_sorting_announcement(guild, member, winning_house)
            return

        next_q = QUESTIONS[session["question_index"]]
        view = QuestionView(self.cog, next_q["options"])
        embed = build_question_embed(session["question_index"], next_q)
        await interaction.response.edit_message(embeds=[embed], content=None, view=view)


class QuestionView(discord.ui.View):
    def __init__(self, cog: "HousingQuizCog", options: list[tuple[str, dict[str, int]]]):
        super().__init__(timeout=600)
        for idx, (label, score_map) in enumerate(options):
            self.add_item(AnswerButton(cog, label, score_map, idx))


class StartHousingQuizButton(discord.ui.Button):
    def __init__(self, cog: "HousingQuizCog"):
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        with self.cog.database.connect() as conn:
            role_repo = GuildRoleRepository(conn)
            role_service = RoleService(role_repo)
            if role_service.member_has_house(interaction.guild, member):
                await interaction.response.send_message(
        ...
        self.cog.active_sessions[member.id] = {
        ...
        question_embed = build_question_embed(0, first_question)
        view = QuestionView(self.cog, first_question["options"])
...
class StartHousingQuizView(discord.ui.View):
    def __init__(self, cog: "HousingQuizCog"):
        ...
        self.add_item(StartHousingQuizButton(cog))


class HousingQuizCog(commands.Cog):
    def __init__(self, bot: commands.Bot, database: Database):
        self.bot = bot
        self.database = database
        self.active_sessions: dict[int, dict] = {}
        self.start_view = StartHousingQuizView(self)
    ...
    def _announcement_key(self, guild_id: int) -> str:
        return f"{BOT_STATE_SORTING_ANNOUNCEMENT_PREFIX}{guild_id}"

    def _quiz_message_key(self, guild_id: int) -> str:
        return f"{BOT_STATE_QUIZ_MESSAGE_PREFIX}{guild_id}"

    def get_sorting_announcement_channel_id(self, guild_id: int) -> int | None:
        with self.database.connect() as conn:
            repo = BotStateRepository(conn)
            value = repo.get_value(self._announcement_key(guild_id))
        return int(value) if value else None

    def set_sorting_announcement_channel_id(self, guild_id: int, channel_id: int | None) -> None:
        with self.database.connect() as conn:
            repo = BotStateRepository(conn)
            repo.set_value(self._announcement_key(guild_id), "" if channel_id is None else str(channel_id))

    def get_quiz_message_data(self, guild_id: int) -> dict[str, int] | None:
        with self.database.connect() as conn:
            repo = BotStateRepository(conn)
            value = repo.get_value(self._quiz_message_key(guild_id))
        if not value:
            return None
        return json.loads(value)

    def set_quiz_message_data(self, guild_id: int, data: dict[str, int] | None) -> None:
        with self.database.connect() as conn:
            repo = BotStateRepository(conn)
            repo.set_value(self._quiz_message_key(guild_id), "" if data is None else json.dumps(data))

    async def send_sorting_announcement(self, guild: discord.Guild, member: discord.Member, house_name: str) -> None:
        channel_id = self.get_sorting_announcement_channel_id(guild.id)
        if not channel_id:
            return
        ...
        try:
            announcement_embed = make_house_embed(
                ...
                description=f"{member.mention} has been placed into **{house_name}**!",
    ...
    async def sethousingquiz(self, interaction: discord.Interaction) -> None:
        ...
        embed = make_base_embed(
            ...
            description=(
                ...
                "The quiz will open privately in this channel so only you can see it."
            ),
        )
        embed.add_field(
        ...
        message = await interaction.channel.send(embed=embed, view=self.start_view)
        self.set_quiz_message_data(
    ...
    async def removehousingquiz(self, interaction: discord.Interaction) -> None:
        if not self.is_admin(interaction):
            await interaction.response.send_message(
                "You do not have permission to use this command.",
                ephemeral=True,
        ...
        data = self.get_quiz_message_data(interaction.guild.id)
        if not data:
            await interaction.response.send_message(
        ...
        if channel is None:
            self.set_quiz_message_data(interaction.guild.id, None)
            await interaction.response.send_message(
    ...
    async def addsortingannouncement(self, interaction: discord.Interaction) -> None:
        if not self.is_admin(interaction):
            await interaction.response.send_message(
                "You do not have permission to use this command.",
                ephemeral=True,
        ...
        self.set_sorting_announcement_channel_id(interaction.guild.id, interaction.channel.id)
        await interaction.response.send_message(
    ...
    async def removesortingannouncement(self, interaction: discord.Interaction) -> None:
        if not self.is_admin(interaction):
            await interaction.response.send_message(
                "You do not have permission to use this command.",
                ephemeral=True,
        ...
        current_channel_id = self.get_sorting_announcement_channel_id(interaction.guild.id)
        ...
        self.set_sorting_announcement_channel_id(interaction.guild.id, None)
        await interaction.response.send_message(
